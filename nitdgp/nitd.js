(function() {
  ...
  */
  ...
  CU.marquee = {
    shim: function(el) {
      var shim;
      shim = el.find('.marquee-img').first();
      shim = shim.removeClass('marquee-img').addClass('marquee-shim').attr('alt', '');
  ...
  */
  ...
  CU.slideshows = {
    init: function(el) {
      ...
      next = el.data('next') ? $(el.data('next')) : $('a.next', controls);
      prev = el.data('prev') ? $(el.data('prev')) : $('a.prev', controls);
...
CU.honeycomb = {
  ...
  slideshow: function() {
	$('.hc-slideshow').scrollface({
	  auto: false,
	  next: $('#honeycomb a.next'),
	  prev: $('#honeycomb a.prev'),
